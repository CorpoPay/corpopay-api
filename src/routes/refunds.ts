  ...
  asyncHandler(async (req, res) => {
    ...
    if (intent.refunds.some((r) => r.status === "SUCCEEDED" || r.status === "PENDING")) {
      throw new AppError(
        409,
        "ALREADY_REFUNDED",
        "A refund for this transaction is already in progress or completed",
      );
    }
    ...
    const amountCentimes = intent.paymentLink
      ? madToCentimes(intent.paymentLink.amount) // MAD → centimes
      : centimes(Number((intent.metadata as any)?.amount ?? 0)); // already centimes
    const amountMad = centimesToMad(amountCentimes);
    ...
    const refund = await prisma.refund
      .create({
        data: {
