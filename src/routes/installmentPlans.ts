...
router.delete(
  ...
  asyncHandler(async (req, res) => {
    ...
    const activeCount = await prisma.installmentAgreement.count({
      where: {
        planId: plan.id,
...
publicInstallmentPlansRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!link.isInstallment) throw new AppError(400, 'NOT_INSTALLMENT', 'This link does not support installments');
