...
const SafeUrl = z.string().url().refine(safeUrl, {
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const callbackUrl =
      body.webhookUrl ?? `${apiBase}/webhooks/${body.provider.toLowerCase()}`;
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!intent)
      throw new AppError(404, "INTENT_NOT_FOUND", "Payment intent not found");
    res.json(intent);
  }),
);
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    try {
      ...
      console.warn(
        "[GET /:id/status] queryTransactionStatus failed — returning DB status",
        {
          intentId: intent.id,
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!amount) {
      ...
      throw new AppError(
        400,
        "MISSING_AMOUNT",
        "Cannot determine amount to capture",
      );
    }
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!intent.providerRef) {
      ...
      throw new AppError(
        400,
        "MISSING_PROVIDER_REF",
        "Intent has no provider reference to cancel",
      );
    }
...
publicRelayRouter.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!intent)
      throw new AppError(404, "INTENT_NOT_FOUND", "Payment session not found");
  ...
  asyncHandler(async (req, res) => {
    ...
    if (link.tenant.status === "DISABLED")
      throw new AppError(
        403,
        "TENANT_DISABLED",
        "Merchant not accepting payments",
      );
    ...
    if (installmentPlanId) {
      ...
      if (!plan)
        throw new AppError(
          404,
          "PLAN_NOT_FOUND",
          "Installment plan not found or not active",
        );
      ...
      const remainingPrincipal = Math.max(
        0,
        Math.round((principal - downPayment) * 100) / 100,
      );
      ...
      const totalInstallments =
        1 + (remainingInstallments > 0 ? remainingInstallments : 0);
      ...
      const agreement = await prisma.installmentAgreement.create({
        data: {
          ...
          downPayment,
          installmentAmount:
            remainingMonthlyAmt > 0 ? remainingMonthlyAmt : downPayment,
          totalInstallments,
