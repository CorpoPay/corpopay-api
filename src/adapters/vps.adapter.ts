...
function generatePaywallSignature(payload: Record<string, unknown>, secretKey: string): string {
  return crypto
    .createHash("sha256")
...
function generateCommandHmac(
  ...
  const message = callerName + merchantAccount + timestamp + requestPath + requestBodyStr;
  // Payzone docs example shows password used as-is (no uppercase) — the phrase
  // "callerPassword in uppercase" in the description is misleading; the concrete
  // example directly below it uses the raw password value.
  return crypto.createHmac("sha256", callerPassword).update(message).digest("hex").toUpperCase();
}
...
export class VpsAdapter implements ProviderAdapter {
  ...
  async createCheckoutSession(params: CreateCheckoutParams): Promise<CreateCheckoutResult> {
    ...
    const payload: Record<string, unknown> = {
      ...
      // Omit customerEmail when absent (like the tutorial) — an empty string
      // can make the paywall block on an email prompt or reject the payload.
    ...
    // when calling /pwthree/api/initialize?mode=DEEP_LINK&... — without it
    // initialize 400s and the paywall never renders (confirmed in sandbox logs).
    return {
  ...
  async queryTransactionStatus(providerRef: string): Promise<QueryStatusResult> {
    ...
    return {
      ...
      providerTransactionId:
        (chargeData["id"] as string | undefined) ??
        (chargeData["transactionId"] as string | undefined),
      rawResponse: raw,
    };
  }
  ...
  verifyWebhookSignature(rawBody: Buffer, headers: Record<string, string>): boolean {
    ...
    const expected = crypto.createHmac("sha256", notificationKey).update(rawBody).digest("hex");
  ...
  mapStatusToInternal(providerStatus: string): PaymentIntentStatus {
    return VPS_STATUS_MAP[providerStatus.toUpperCase()] ?? "PROCESSING";
  }
  ...
  private async runCommand(
    ...
    if (!response.ok) {
      throw new Error(
        `VPS command ${command} HTTP ${response.status}: ${raw.message ?? JSON.stringify(raw)}`,
      );
    }
