...
export class VpsAdapter implements ProviderAdapter {
  ...
  async createCheckoutSession(
    ...
    const payload: Record<string, unknown> = {
      ...
      showPaymentProfiles: params.storePaymentProfile
        ? "true"
        : (c.showPaymentProfiles ?? "false"),
  ...
  async queryTransactionStatus(
    ...
    if (!response.ok) {
      throw new Error(
        `VPS GET HTTP ${response.status}: ${JSON.stringify(raw)}`,
      );
    }

    // Support both flat { status } and wrapped { charge: { status } } responses
    const chargeData =
      (raw["charge"] as Record<string, unknown> | undefined) ?? raw;
    const providerStatus = (
      (chargeData["status"] as string) ?? "UNKNOWN"
    ).toUpperCase();
