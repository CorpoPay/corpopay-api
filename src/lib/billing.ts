...
/**
 * Given a principal, APR, and number of months, returns a full amortization
 * schedule as an array of installment amounts (length = n).
 * The last installment is adjusted so total collected == total payable.
 */
export function buildAmortizationSchedule(
  ...
  const schedule = Array(n - 1).fill(monthly);
  const lastPayment = Math.round((principal + totalInterest(principal, annualRatePct, n) - monthly * (n - 1)) * 100) / 100;
  schedule.push(Math.max(0, lastPayment));
  return schedule;
}
