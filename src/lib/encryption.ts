...
export function decrypt(ciphertext: string): string {
  ...
  if (!legacyRaw) throw new Error('ENCRYPTION_KEY_V1 required to decrypt legacy credentials. Set it and re-run the migration.');
  ...
  const result   = CryptoJS.AES.decrypt(params, key, { iv, mode: CryptoJS.mode.CBC, padding: CryptoJS.pad.Pkcs7 });
