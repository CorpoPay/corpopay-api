...
async function main() {
  ...
  // ─── Jabadoor tenant ──────────────────────────────────────────────────────────────
  let jabadoor = await prisma.tenant.findUnique({
    where: { slug: "jabadoor" },
  });
  ...
  await prisma.providerConfig.upsert({
    where: {
      tenantId_provider: { tenantId: jabadoor.id, provider: Provider.VPS },
    },
    create: {
      tenantId: jabadoor.id,
  ...
  for (const p of jabadoorPlans) {
    ...
    if (!existing) {
      ...
      console.log(
        `⏭  Plan "${p.name}" updated to ${p.annualInterestRate}% APR.`,
      );
    }
  }
